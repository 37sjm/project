...
--inner join (default)
select Movie.title from Movie, Casting where Movie.id = Casting.movieid;

--outer join (includes all from performers)
select Performer.firstname from (Performer Left outer join Casting on casting.performerid = performer.id);

--aggregate function
select avg(score) from Movie;

--group by example
select max(score) from Movie group by Title;